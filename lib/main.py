...
def load_rec_data(db):
    ...
    for idx, centroid_data in enumerate(centroids_data):
        centroids[centroid_data['_id']] = np.array(centroid_data['data'])
...
def assign_user_to_cluster(user_weights, centroids):
    user_cluster = centroids[centroids.keys()[0]]
    min_dist = np.linalg.norm(user_weights - centroids.values()[0])
    for idx, centroid in enumerate(centroids.values()):
        if idx != 0:
            dist = np.linalg.norm(user_weights - centroid)
            if dist < min_dist:
                min_dist = dist
                min_dist_idx = idx
                user_cluster = centroids[centroids.keys()[min_dist_idx]]
    return centroids.keys()[min_dist_idx], user_cluster


def get_user_weights_cluster(db, centroids, userId):
    ...
    if user_data:
        ...
        if assignment_data:
            ...
            return np.array(user_data['data']), centroid_data
        else:
            user_weights = np.array(user_data['data'])
            centroid_id, centroid_data = assign_user_to_cluster(user_weights, centroids)
            db.assignments.insert_one({'userId': userId, 'centroidId': centroid_id})
            return np.array(user_data['data']), centroid_data
    else:
        return None, None
...
def recommended(userId):
    ...
    user_weights, centroid_data = get_user_weights_cluster(rec_db, centroids, userId)
    if user_weights is None or centroid_data is None:
        return None
    ...
    sorted_recommendation = sorted(recommendation.items(), key=lambda x: -x[1])
